...
import { parsePartnerViewSelection, platformsForSelection } from "./partner-view-metrics";
...
const MAX_PAGES = 10; // 1000 records max per record kind (per platform) - a deliberate, honest bound, never a true fetch-all.
...
function fetchBoundedPartnerRecords(actor: ActorContext, recordKind: "content", partnerRef: string, platform: PlatformViewId): Promise<BoundedRecords<AnalyticsContentSourceRecordDoc>>;
function fetchBoundedPartnerRecords(actor: ActorContext, recordKind: "channel", partnerRef: string, platform: PlatformViewId): Promise<BoundedRecords<AnalyticsChannelSourceRecordDoc>>;
async function fetchBoundedPartnerRecords(actor: ActorContext, recordKind: "content" | "channel", partnerRef: string, platform: PlatformViewId): Promise<BoundedRecords<AnalyticsContentSourceRecordDoc | AnalyticsChannelSourceRecordDoc>> {
...
export async function getPartnerAnalyticsView(actor: ActorContext | null, partnerRefInput: unknown, platformInput?: unknown): Promise<AnalyticsServiceResult<PartnerAnalyticsViewDto>> {
  ...
  return { ok: true, data: buildPartnerAnalyticsView({ partnerRef: partner.partnerRef, partnerDisplayName: partner.displayName, selected, coverage, inputs }) };
}
