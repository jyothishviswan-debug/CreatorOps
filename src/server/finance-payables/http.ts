...
// Session/actor resolution and request-id generation are generic, not Payables-specific - reused
// directly, same as Finance Agreements'/Partner Reviews' own http.ts.
export { resolveRequestActor, newRequestId, parseJsonBody } from "@/server/administration/http";

import type { FinancePayablesServiceErrorCode, FinancePayablesServiceResult } from "./types";
...
// its blockers) / 500. Identical mapping to the Finance Agreements one.
export function toFinancePayablesHttpResponse<T>(result: FinancePayablesServiceResult<T>, successStatus = 200): NextResponse {
  ...
  const statusByCode: Record<Exclude<FinancePayablesServiceErrorCode, "unauthorized" | "not_ready">, number> = {
    not_found: 404,
    invalid_input: 400,
    stale_write: 409,
    conflict: 409,
