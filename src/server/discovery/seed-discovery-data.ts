...
export async function seedDiscoveryData(): Promise<void> {
  ...
  function base(uid: string, leadRef: string, displayName: string): Omit<LeadDoc, "lifecycle" | "previousLifecycle" | "lifecycleReason" | "regionIds" | "teamId" | "ownerUid"> {
    return {
      uid,
      leadRef,
      version: 1,
  ...
  const leads: LeadDoc[] = [
    ...
    },
    {
      ...
      teamId: null,
      ownerUid: null,
      research: { targetAudience: ["India 1"], language: "Hindi", notes: "Strong regional storytelling niche.", updatedAt: now, updatedByUserRef: headUserRef },
    },
    {
    ...
    },
    {
      ...
      teamId: null,
      ownerUid: headUid,
      // A genuinely multi-value fixture - carries both values through
      // conversion into seed-partner-converted below.
      research: { targetAudience: ["India Alpha", "India 1"], updatedAt: now, updatedByUserRef: headUserRef },
      outreachSummary: { totalCount: 3, lastDirection: "INBOUND", lastChannel: "call", lastOutcome: "Confirmed interest", lastAt: now, nextFollowUpAt: null },
      respondedAt: now,
  ...
  const partner: PartnerDoc = {
    ...
    previousStatus: null,
    statusReason: null,
    regionIds: convertedLead.regionIds,
    languageIds: [],
    categoryIds: [],
    tier: null,
    priority: null,
    targetAudience: convertedLead.research?.targetAudience ?? [],
