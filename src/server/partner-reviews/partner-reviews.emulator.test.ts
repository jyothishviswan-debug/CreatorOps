...
const FINANCE_AGREEMENT_ROOTS = /^finance(Agreements|AgreementClaims|ContractArtifacts|AgreementRestrictedExtractions|Payables|Invoices|InvoiceNumberClaims|Payments|PaymentReferenceClaims|PaymentSettlements)$/; // Step 14A: the Agreement foundation legitimately owns these roots (Partner Reviews writing them is proven by the write-instrumentation tests); anything else Finance-shaped is still a violation. // Step 15A: financePayables joins the list - it is a legitimate Finance root that OTHER emulator test files (which run in parallel against the one emulator) may first-write during this test. // Step 16A: financeInvoices / financeInvoiceNumberClaims join it the same way, owned by the Invoices module. // Step 17A: financePayments / financePaymentReferenceClaims / financePaymentSettlements join it the same way, owned by the Payments module (Step 17C found and closed this gap - it was never added when Step 17A landed). What this file actually proves is unchanged and still exact: the write-instrumentation assertions below show Partner Reviews itself writes NO Finance document of any kind.
