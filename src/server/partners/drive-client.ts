...
// The shared Drive folder every Partner's own evidence subfolder is
// created inside - a dedicated "Partners" subfolder that already lives
// directly under Discovery's own shared root (same parent as its
// "Vendors" sibling - see vendors/drive-client.ts), inheriting that
// root's sharing with the service account automatically. This is what
// satisfies "for discovery and partners, use the same folder": every
// domain's evidence lives under the one shared Drive tree, organized
// into its own clean subfolder rather than mixed in with Discovery's own
// Lead folders directly at the root.
const ROOT_FOLDER_ID = "1J4Qp0yDmNrf-GSwawAf43rd-ulHxOqzA";
...
// "P01_Partner Name_" - the Partner-side parallel to Vendor's own
// business-specified "V01_Vendor Name_" pattern (a 2-digit-minimum,
// zero-padded sequence number - see types.ts's sequenceNumber field),
// never re-derived from anything else once allocated.
export function partnerDriveFolderName(sequenceNumber: number, displayName: string): string {
...
export async function ensurePartnerDriveFolder(name: string): Promise<DriveResult<{ folderId: string }>> {
  try {
...
export async function uploadFileToPartnerDriveFolder(
  folderId: string,
