...
// Campaigns-specific - reused directly, same as Vendors'/Partners' own
// http.ts.
export { resolveRequestActor, newRequestId, parseJsonBody } from "@/server/administration/http";

import type { CampaignsServiceErrorCode, CampaignsServiceResult } from "./types";

export function toCampaignsHttpResponse<T>(result: CampaignsServiceResult<T>, successStatus = 200): NextResponse {
  ...
  const statusByCode: Record<Exclude<CampaignsServiceErrorCode, "unauthorized" | "not_ready">, number> = {
    not_found: 404,
    invalid_input: 400,
    stale_write: 409,
    internal: 500,
