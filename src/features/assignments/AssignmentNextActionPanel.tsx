...
//
// Step 11A.1: IN_PROGRESS's Complete button is real (the trusted
// fulfillment evaluator now asks a single question: has this
// Assignment's one canonical Content thread reached APPROVED) - it
// renders only once the shared useAssignmentContentFulfillment hook
// reports the thread APPROVED; until then, a neutral truthful status
// line is shown instead of inviting a guaranteed-to-fail click. There is
// no separate "Complete Content" step anywhere - approving the thread IS
...
export function AssignmentNextActionPanel({ assignment, onSaved }: { assignment: AssignmentDto; onSaved: (assignment: AssignmentDto) => void }) {
  ...
  // Step 11A.1: IN_PROGRESS's own completion picture is fetched via the
  // SAME shared single-thread-status hook AssignmentContentPanel uses,
  // never a second independent implementation. Only enabled while
  // IN_PROGRESS, to avoid an unnecessary read on every other status.
  const fulfillment = useAssignmentContentFulfillment(assignment.assignmentRef, assignment.status === "IN_PROGRESS");
        ...
        {next ? (
          ...
          fulfilled ? (
            <>
              ...
              <p className="detailcopy" style={{ margin: "8px 0 17px" }}>
                The submission thread is Approved. Server remains authoritative - this action is re-verified on every attempt.
              </p>
            ...
            <>
              ...
              <p className="detailcopy" style={{ margin: "8px 0 0" }}>
                {fulfillment.loading ? "Loading Content fulfillment…" : fulfillment.thread ? `Submission thread is ${fulfillment.thread.status.replace("_", " ").toLowerCase()}` : "No submission thread yet"}
              </p>
            </>
          )
        ) : (
