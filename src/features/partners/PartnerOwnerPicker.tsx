...
import { searchPartnerOwnerCandidates } from "./api-client";
...
// /api/partners/users/search endpoint, same idiom as Discovery's own
// ManagerPicker.
export function PartnerOwnerPicker({ onSelect, placeholder = "Search by email…" }: { onSelect: (candidate: PartnerOwnerCandidateDto) => void; placeholder?: string }) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<PartnerOwnerCandidateDto[]>([]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      setSearching(true);
      const result = await searchPartnerOwnerCandidates(trimmed.toLowerCase());
      if (cancelled) return;
      setSearching(false);
      if (!result.ok) {
