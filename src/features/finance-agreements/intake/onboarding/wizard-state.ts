...
import { duplicateSignature, emptyForm, prefillFromPreview, reshapeAccounts, setFormField, validateForm, type FormFieldKey, type FormIssue, type OnboardingForm } from "./wizard-form";
...
export function wizardReducer(state: WizardState, action: WizardAction): WizardState {
  switch (action.type) {
    ...
    case "edit": {
      if (isLocked(state)) return state;
      const form = setFormField(state.form, action.field, action.value);
      ...
      return { ...state, form, touched: state.touched.includes(action.field) ? state.touched : [...state.touched, action.field], create };
    }
    case "showIssues":
