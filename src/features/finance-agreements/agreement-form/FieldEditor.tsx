...
// Step 14C.3: the value editors, one per editor kind, built only from the Foundation `.field` archetype (a real
// label bound by htmlFor/id, one control, a message under it). Every editor reports through onChange(next, immediate).
...
import { splitPlatformText, type FieldEditorState } from "../agreement-intake-logic/editors/editor-state";
...
function SimpleEditor({ fieldKey, kind, label, state, onChange, onBlur, errors, disabled = false }: FieldEditorProps) {
  ...
  if (state.kind === "boolean") {
    control = (
      <select {...common} style={FULL_WIDTH} value={state.value === null ? "" : state.value ? "yes" : "no"} onChange={(event) => onChange({ kind: "boolean", value: event.target.value === "" ? null : event.target.value === "yes" }, true)}>
    ...
    if (kind === "longText") {
      ...
      control = <input {...common} type="text" inputMode={fieldKey === "contactNumber" ? "tel" : fieldKey === "emailAddress" ? "email" : undefined} style={FULL_WIDTH} autoComplete="off" value={state.text} onChange={(event) => set(event.target.value)} data-field={fieldKey} />;
    }
  } else {
    control = null;
  }
