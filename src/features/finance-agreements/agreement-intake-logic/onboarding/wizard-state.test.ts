...
describe("typing a page link suggests the page/account name", () => {
  const locator = accountFieldKey(0, "locator");
  const locatorKind = accountFieldKey(0, "locatorKind");
  const pageName = accountFieldKey(0, "pageName");

  it("fills the name from the link while the reviewer has not typed a name themselves", () => {
    const state = run(START, { type: "pickFile", file: FILE, error: null }, { type: "edit", field: locator, value: "https://www.instagram.com/_mrinal_speaks" });
    expect(state.form.accounts[0]!.pageName).toBe("_mrinal_speaks");
    expect(state.touched).not.toContain(pageName); // a suggestion, not the reviewer's own entry
  });

  it("never overrides a name the reviewer already typed", () => {
    let state = run(START, { type: "edit", field: pageName, value: "My Own Name" });
    state = run(state, { type: "edit", field: locator, value: "https://www.instagram.com/_mrinal_speaks" });
    expect(state.form.accounts[0]!.pageName).toBe("My Own Name");
  });

  it("keeps tracking the (untouched) name as the link changes, and clears it for a link with no readable name", () => {
    let state = run(START, { type: "edit", field: locator, value: "https://www.instagram.com/first_handle" });
    expect(state.form.accounts[0]!.pageName).toBe("first_handle");
    state = run(state, { type: "edit", field: locator, value: "https://www.instagram.com/second_handle" });
    expect(state.form.accounts[0]!.pageName).toBe("second_handle");
    state = run(state, { type: "edit", field: locator, value: "https://www.instagram.com/p/Cabc123" }); // a post, not a profile
    expect(state.form.accounts[0]!.pageName).toBe("");
  });

  it("suggests nothing while the row is identified by a Handle, not a link", () => {
    const state = run(START, { type: "edit", field: locatorKind, value: "HANDLE" }, { type: "edit", field: locator, value: "https://www.instagram.com/_mrinal_speaks" });
    expect(state.form.accounts[0]!.pageName).toBe("");
  });

  it("once the reviewer types into the name field directly, later link edits leave it alone even if they clear it back to blank", () => {
    let state = run(START, { type: "edit", field: locator, value: "https://www.instagram.com/first_handle" });
    expect(state.form.accounts[0]!.pageName).toBe("first_handle"); // still a suggestion
    state = run(state, { type: "edit", field: pageName, value: "" }); // the reviewer deliberately clears it
    expect(state.touched).toContain(pageName);
    state = run(state, { type: "edit", field: locator, value: "https://www.instagram.com/second_handle" });
    expect(state.form.accounts[0]!.pageName).toBe(""); // stays exactly what the reviewer left it as
  });

  it("editing another row's locator never touches this row's name", () => {
    const twoRows = initialWizardState("PARTNER", ["instagram", "youtube"]);
    const state = run(twoRows, { type: "edit", field: accountFieldKey(1, "locator"), value: "https://www.youtube.com/@somechannel" });
    expect(state.form.accounts[0]!.pageName).toBe("");
    expect(state.form.accounts[1]!.pageName).toBe("@somechannel");
  });
});
