...
import { searchVendorOwnerCandidates } from "./api-client";
...
// /api/vendors/users/search endpoint, same idiom as Partners' own
// PartnerOwnerPicker.
export function VendorOwnerPicker({ onSelect, placeholder = "Search by email…" }: { onSelect: (candidate: VendorOwnerCandidateDto) => void; placeholder?: string }) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<VendorOwnerCandidateDto[]>([]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      setSearching(true);
      const result = await searchVendorOwnerCandidates(trimmed.toLowerCase());
      if (cancelled) return;
      setSearching(false);
      if (!result.ok) {
