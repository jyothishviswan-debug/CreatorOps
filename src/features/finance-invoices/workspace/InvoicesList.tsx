...
import type { InvoiceRowDto, InvoiceWorkspaceDto } from "@/server/finance-invoices/client-dto";

import { loadInvoicesWorkspace } from "../api-client";
import { InvoiceCards, InvoicesTable } from "./InvoiceRows";
...
import { NEW_INVOICE_HREF, toWorkspaceRowView } from "./workspace-view-model";
import type { WorkspaceLayout } from "./WorkspaceToolbar";

// Step 16B: one filter state's list of Invoice rows. It is REMOUNTED (React `key` = the canonical
...
export function InvoicesList({ initial, state, layout, anyFilterActive, onClearFilters }: { initial: InvoiceWorkspaceDto; state: WorkspaceUrlState; layout: WorkspaceLayout; anyFilterActive: boolean; onClearFilters: () => void }) {
  const [pages, setPages] = useState<InvoiceRowDto[][]>([initial.rows]);
  ...
  async function goToPage(page: number) {
    ...
    setLoading(true);
    setError(null);
    const result = await loadInvoicesWorkspace(toWorkspaceRequest(state, { cursor }), { signal: controller.signal });
  ...
  const noInvoicesAtAll = !anyFilterActive && !disclosure.headsTruncated;
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          action={
            ...
            ) : noInvoicesAtAll && permissions.canManage ? (
        ...
        <InvoiceCards rows={views} />
      ) : (
        <InvoicesTable rows={views} />
      )}
