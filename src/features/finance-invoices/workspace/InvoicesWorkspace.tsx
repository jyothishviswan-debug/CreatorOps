...
import type { InvoiceWorkspaceDto } from "@/server/finance-invoices/client-dto";
...
// Step 16B: the Invoices workspace. The SERVER page resolves the actor, parses the URL into
// `state`, reads the first bounded page through listInvoicesWorkspace and hands both here. Every
...
// toolbar, the status strip) is NOT keyed, so typing in the counterparty box keeps focus across
// the refresh.
export function InvoicesWorkspace({ initial, state, ignoredFilterCount = 0 }: { initial: InvoiceWorkspaceDto; state: WorkspaceUrlState; ignoredFilterCount?: number }) {
    ...
    <>
      ...
      <section className="panel" aria-busy={pending}>
        ...
        {pending || layout === null ? (
          ...
          <InvoicesList key={workspaceStateKey(state)} initial={initial} state={state} layout={layout} anyFilterActive={anyFilterActive} onClearFilters={clearFilters} />
        )}
      </section>
    </>
  );
...
// Step 16B section 4: one compact horizontal status strip (never large dashboard cards) showing
// Draft / Submitted / Approved / Rejected / Void, computed from the current bounded/filtered read.
function StatusStrip({ counts, truncated }: { counts: { draft: number; submitted: number; approved: number; rejected: number; void: number }; truncated: boolean }) {
    ...
    <div className="panel" style={{ marginBottom: 14 }} data-testid="workspace-status-strip">
      ...
      {truncated && (
        <p className="foundationnote" style={{ padding: "0 18px 12px", margin: 0 }}>
          Counts are of the Invoices read, not a total.
        </p>
      )}
    </div>
  );
