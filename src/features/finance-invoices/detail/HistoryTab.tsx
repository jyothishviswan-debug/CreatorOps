...
import { listInvoiceEvents } from "../api-client";
import { historyRows } from "./detail-view";

// Step 16B: Invoice detail - History tab. A compact chronological table, fetched on demand (only
// when this tab is actually viewed) rather than on every detail-page load.
export function HistoryTab({ invoiceRef }: { invoiceRef: string }) {
  const [rows, setRows] = useState<ReturnType<typeof historyRows> | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const controller = new AbortController();
    void listInvoiceEvents(invoiceRef, {}, { signal: controller.signal }).then((result) => {
      if (!result.ok) {
