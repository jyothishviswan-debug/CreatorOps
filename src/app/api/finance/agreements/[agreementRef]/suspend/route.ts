...
import { suspendAgreement } from "@/server/finance-agreements";
...
// POST /api/finance/agreements/[agreementRef]/suspend - ACTIVE -> SUSPENDED {expectedDocVersion (the HEAD's), reason}; needs activate_agreements
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const result = await suspendAgreement(actor, { ...(body as object), agreementRef }, newRequestId());
