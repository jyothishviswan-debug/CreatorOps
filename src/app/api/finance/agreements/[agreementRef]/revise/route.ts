...
import { createAgreementRevision } from "@/server/finance-agreements";
...
// POST /api/finance/agreements/[agreementRef]/revise - start the next draft version from the governing one {expectedDocVersion (the HEAD's)}; needs activate_agreements
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const result = await createAgreementRevision(actor, { ...(body as object), agreementRef }, newRequestId());
