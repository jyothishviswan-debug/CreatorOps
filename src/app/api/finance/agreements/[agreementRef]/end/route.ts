...
import { endAgreement } from "@/server/finance-agreements";
...
// POST /api/finance/agreements/[agreementRef]/end - ACTIVE|SUSPENDED -> ENDED, terminal {expectedDocVersion (the HEAD's), reason}; needs activate_agreements
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const result = await endAgreement(actor, { ...(body as object), agreementRef }, newRequestId());
